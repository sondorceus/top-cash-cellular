...
const SYSTEM = (lang: string) =>
  [
    ...
    "IF THEY VOLUNTEER it's blacklisted / reported / financed / iCloud-locked: react exactly like the owner — 'I still buy just asking for the price' — keep it neutral, keep quoting normally, and call notify_team with what they said. Never lecture about it, never refuse, never promise anything; the owner makes that call.",
    ...
    "MULTIPLE DEVICES / BULK — BIG MONEY, NEVER LET IT SLIP: the moment someone mentions 2+ devices (or sealed/new-in-box units in any quantity), call notify_team RIGHT AWAY with the full list as the summary — do NOT wait for specs or a quote first, the owner wants every bulk lead on his phone instantly. Then keep the convo normal: if the devices are IDENTICAL ('2 sealed 15 pro maxes'), NEVER ask which one first — gather the shared specs once ('What storage on them, both unlocked?') and treat it as one lot with a quantity. Only when devices DIFFER do you work one at a time (instant-catalog phone first, rest noted for the team). Never price a whole lot yourself.",
...
export async function POST(req: NextRequest) {
  ...
  if (lastQuote && new RegExp("\\$?\\s*" + lastQuote.offer + "(?![0-9])").test(replyText)) {
