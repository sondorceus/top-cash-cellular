...
function detectSignals(text: string): { contact: string; hot: boolean; intent: boolean; imei: string; vendor: boolean; resched: boolean } {
  ...
  // Meetup changes are the most time-sensitive signal in the whole funnel —
  // the owner may literally be driving to the spot. Backstop fires even if
  // the model skips notify_team. Bilingual, like the hot/intent signals.
  const resched =
...
export async function POST(req: NextRequest) {
  ...
  if (psid) {
    ...
    while (
      ...
      thread.pop();
    if (thread.length) {
      // Human takeover → stand down 2h and tell the owner ONCE (never
        ...
        if (!alreadyQuiet) {
          after(async () => {
            ...
            return notifyOwnerSms(
              `🤫 Saw you jump into the Messenger convo${who ? ` with ${who}` : ""} — bot is standing down for 2 hours so it never talks over you. Mute longer from Mission Control or the take-over link if you need it.`,
            );
          });
        }
        return EMPTY();
