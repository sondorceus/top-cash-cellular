...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = isHumanHandoff
    ...
    : [
        ...
        "MULTIPLE DEVICES (2 or more): our team prices lots directly and these are our best sellers to work with. The MOMENT you learn it's more than one device, ask ONCE for their phone number — 'more than one, got it — drop your number so our team can text you the combined offer, then let's run through them.' If they don't give it, do NOT ask again right away and NEVER withhold a price for it: keep going, quote each device with get_quote as its specs land, and keep a short running recap as numbers land ('so far: 13 Pro 256 good $430 + S22 cracked $95 — $525 total so far'). Ask for the number again only at a natural close point (after the recap/total, or when handing off), once. The itemized sum is real; anything beyond it is the owner's call — NEVER name a package price or bundle discount, and never close the lot yourself. When the list is done — or the seller slows down, gives partial answers, or goes quiet — call notify_team with whatever you have (itemized list + their number if you have it); missing specs are fine, the team fills gaps by text. End concretely: the team will text their combined offer with the standard follow-up timing.",
  ...
  try {
    ...
    const dynamicSys = [
      isLot ? "THIS CONVERSATION IS A MULTI-DEVICE LOT. Ask once for their phone number if you don't have it; whether or not they give it, quote every device with get_quote as its specs arrive and keep a running recap, then hand off to our team via notify_team. Never hold a price back for a phone number. Do NOT close the lot, do NOT name a package price." : "",
