    ...
    <main className="min-h-screen bg-white text-[#0a0a0a]">
      <nav className="sticky top-0 z-40 bg-white/85 backdrop-blur-xl border-b border-black/15">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <Link href="/" className="text-xs text-[#888] hover:text-[#0a0a0a]">← Single device?</Link>
        </div>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-10 pb-16">
        ...
        <div className="grid grid-cols-3 gap-2 mb-8">
          <div className="bg-black/5 border border-black/15 rounded-xl p-4 text-center">
            <div className="text-2xl mb-1">📦</div>
            <p className="text-[10px] font-bold text-[#888] uppercase">Free pickup</p>
            <p className="text-xs text-[#0a0a0a] mt-1">Austin metro area</p>
          </div>
          <div className="bg-black/5 border border-black/15 rounded-xl p-4 text-center">
            <div className="text-2xl mb-1">⚡</div>
            <p className="text-[10px] font-bold text-[#888] uppercase">24-hr payout</p>
            <p className="text-xs text-[#0a0a0a] mt-1">After verification</p>
          </div>
          <div className="bg-black/5 border border-black/15 rounded-xl p-4 text-center">
            <div className="text-2xl mb-1">🔒</div>
            <p className="text-[10px] font-bold text-[#888] uppercase">Data wipe</p>
            <p className="text-xs text-[#0a0a0a] mt-1">NIST 800-88</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-[#00c853]/8 via-transparent to-[#00c853]/8 border border-[#00c853]/25 rounded-2xl p-5 mb-8">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-black/5 border border-black/15 rounded-xl px-3 py-3 text-center">
              <p className="text-2xl font-extrabold text-[#0a0a0a]">+5%</p>
            ...
            <div className="bg-[#00c853]/10 border border-[#00c853]/30 rounded-xl px-3 py-3 text-center">
              ...
              <p className="text-[11px] text-[#555] mt-0.5 leading-tight">50+ devices</p>
            </div>
            <div className="bg-black/5 border border-black/15 rounded-xl px-3 py-3 text-center">
              <p className="text-lg font-extrabold text-[#0a0a0a] leading-none mt-1">Custom</p>
        ...
        {submitted ? (
          ...
          <div className="bg-black/5 border border-black/15 rounded-2xl p-6 space-y-4">
            <h2 className="text-lg font-bold mb-2">Tell us about your devices</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name *" className="px-4 py-3 bg-black/5 border border-black/15 rounded-xl text-sm text-[#0a0a0a] placeholder:text-[#777] focus:outline-none focus:border-[#00c853]" />
              <input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Company (optional)" className="px-4 py-3 bg-black/5 border border-black/15 rounded-xl text-sm text-[#0a0a0a] placeholder:text-[#777] focus:outline-none focus:border-[#00c853]" />
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email *" className="px-4 py-3 bg-black/5 border border-black/15 rounded-xl text-sm text-[#0a0a0a] placeholder:text-[#777] focus:outline-none focus:border-[#00c853]" />
              <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone (recommended)" className="px-4 py-3 bg-black/5 border border-black/15 rounded-xl text-sm text-[#0a0a0a] placeholder:text-[#777] focus:outline-none focus:border-[#00c853]" />
            </div>
            <input value={count} onChange={(e) => setCount(e.target.value)} placeholder="Approx number of devices (e.g. 25)" className="w-full px-4 py-3 bg-black/5 border border-black/15 rounded-xl text-sm text-[#0a0a0a] placeholder:text-[#777] focus:outline-none focus:border-[#00c853]" />
            <textarea value={details} onChange={(e) => setDetails(e.target.value)} required rows={5} placeholder="Mix of devices? Brands, models, conditions — anything you can describe. e.g. '20 iPhone 13 Pro 256GB, 10 MacBook Air M2, all flawless.'" className="w-full px-4 py-3 bg-black/5 border border-black/15 rounded-xl text-sm text-[#0a0a0a] placeholder:text-[#777] focus:outline-none focus:border-[#00c853] resize-none" />
            ...
            <p className="text-xs text-[#888] text-center">Or email us a list at <a href="mailto:topcashcellular@gmail.com" className="text-[#0a0a0a] hover:text-[#00c853] transition">topcashcellular@gmail.com</a> for the fastest response.</p>
          </div>
        )}
      </div>
    </main>
