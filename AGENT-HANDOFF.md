...
| `/go` ad funnel | Meta-ads landing page: category tiles → guided quote → one-field lock → meet/ship chips. Live owner takeover at `/admin/chats` (`app/lib/gochat-store.ts`, blob prefix `gochat/`). Since 2026-09-11 a lock TEXTS the seller (Telnyx relay, `app/lib/seller-sms.ts`) with a deep link; replies land in the thread via `/api/go/sms-inbound`, which honors STOP and MEET/SHIP. The reminders cron is relay-based and /go-aware (24h nudge, day-13 expiry, chat-contact follow-up). Funnel numbers: `/admin/analytics` → "/go ad funnel". Ads-side playbook: `docs/go-ads-playbook.md`. The Facebook Messenger bot it replaced was retired 2026-08-19 — recover it from git history if ever needed. |
